...
int main(int argc, char *argv[]) {
  ...
  socklen_t src_addr_len = sizeof(src_addr);
	...
	while (!quit) {
		n = recvfrom(sock, &RecvMsg, sizeof(RecvMsg), 0,(struct sockaddr*)&src_addr, &src_addr_len); // 클라이언트로부터 메시지 수신
		struct KVS SendMsg = {0,};
		if (n > 0){
			int index = hash64(RecvMsg.key) % DATASET_SIZE; // 인덱스 값 계산
			if(RecvMsg.type == READ_REQ) {  // 읽기 요청인 경우
				printf("Type: READ_REQ Key: %s Value: %s\n",RecvMsg.key, RecvMsg.value); // 수신한 내용 출력
				strcpy(RecvMsg.value, kv[index]);
				SendMsg.type = READ_REP; // 응답 타입을 읽기 응답으로 설정
				sendto(sock,&RecvMsg,sizeof(SendMsg),0,(struct sockaddr*)&src_addr, src_addr_len); // 클라이언트에 응답 전송
			}
			else if (RecvMsg.type == WRITE_REQ) { // 쓰기 요청인 경우
				printf("Type: WRITE_REQ Key: %s Value: %s\n",RecvMsg.key, RecvMsg.value); // 수신한 내용 출력
				strcpy(kv[index], RecvMsg.value); // 받은 value를 저장소 인덱스에 복사
				SendMsg.type = WRITE_REP; // 응답 타입을 쓰기 응답으로 설정
				n = sendto(sock, &RecvMsg, 17, 0, (struct sockaddr*)&src_addr, src_addr_len); // 클라이언트에 응답 전송
			}
		}
	}
