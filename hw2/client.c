...
int main(int argc, char *argv[]) {
  ...
  while (cnt < 5) {
    ...
    if (rand() % 2 == 0) {
        ...
        strncpy(SendMsg.value, "AAAABBBBCCCCDDD", VALUE_SIZE - 1); // 최대 길이 - 1만큼 복사
        SendMsg.value[VALUE_SIZE - 1] = '\0'; // null 문자 종료 보장
    }

    // 파티션 번호 설정, 서버 수로 모듈러 연산
    uint64_t hash = hash64(SendMsg.key); 
    int partition = hash % NUM_SRV;
    printf("Partition: %d\n", partition); // 파티션 번호 출력

    // 포트 번호 설정, 5001에 파티션 번호를 더한 값으로 설정
    srv_addr.sin_port = htons(5001 + partition);

    int sentBytes = 0;

    if (SendMsg.type == WRITE_REQ) {
      sentBytes = sendto(sock, &SendMsg, sizeof(SendMsg), 0, (struct sockaddr *)&srv_addr, sizeof(srv_addr)); // write 요청 전송
    }  else {
      sentBytes = sendto(sock, &SendMsg, 17, 0, (struct sockaddr *)&srv_addr, sizeof(srv_addr)); // read 요청 전송, value값을 보낼 필요가 없으므로 크기를 17바이트로 설정
    }

    printf("Sent bytes: %d\n", sentBytes);

    // 요청 출력
    const char* type_str = (SendMsg.type == READ_REQ) ? "READ_REQ" : "WRITE_REQ";
    printf("Type: %s Key: %s Value: %s", type_str, SendMsg.key,SendMsg.value);
    printf("\n");

    // 서버로부터 답장을 수신
    int recvBytes = recvfrom(sock, &RecvMsg, sizeof(RecvMsg), 0, (struct sockaddr *)&src_addr, &src_addr_len);
    printf("Received bytes: %d\n", recvBytes); // 받은 패킷 크기 출력
    if (recvBytes > 0) { // 수신한 데이터가 있다면
        char* type;
        ...
        printf("Type: %s Key: %s Value: %s\n",type,RecvMsg.key, RecvMsg.value); // 수신한 내용 출력
    }
	close(sock); // 소켓을 닫아줌
	return 0;
  }
