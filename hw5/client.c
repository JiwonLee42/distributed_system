...
int main(int argc, char *argv[]) {
  ...
  while (cnt < 5) {
    ...
    if (rand() % 2 == 0) {
      ...
      SendMsg.value[0] = '\0'; 
      sentBytes = sendto(sock, &SendMsg, 17, 0, (struct sockaddr *)&srv_addr, sizeof(srv_addr));
  } else {
      SendMsg.type = WRITE_REQ;
      strncpy(SendMsg.value, "AAAABBBBCCCCDDD", VALUE_SIZE - 1);
      SendMsg.value[VALUE_SIZE - 1] = '\0';
      sentBytes = sendto(sock, &SendMsg, sizeof(SendMsg), 0, (struct sockaddr *)&srv_addr, sizeof(srv_addr));
  }  
    printf("Sent bytes: %d\n", sentBytes);
