...
int main(int argc, char *argv[]) {
  ...
  socklen_t src_addr_len = sizeof(src_addr);
	...
	while (!quit) {
		...
		if (rand() % 2 == 0) { // 50% 확률로 랜덤 설정
			strncpy(SendMsg.value, "CACHECACHECACHE", VALUE_SIZE - 1); // 응답의 value 설정
			SendMsg.type = CACHE_HIT; // 캐시 히트로 타입 설정
			printf("Cache Hit for Key: %s\n", RecvMsg.key); // 캐시 히트 메시지 출력
		} else { // 캐시 미스인 경우
			SendMsg.type = CACHE_MISS; // 캐시 미스로 타입 설정
			printf("Cache Miss for Key: %s\n", RecvMsg.key); // 캐시 미스 메시지 출력
		}
		sendto(sock,&SendMsg,sizeof(SendMsg),0,(struct sockaddr*)&src_addr, src_addr_len); // 클라이언트에 응답 전송
	}
