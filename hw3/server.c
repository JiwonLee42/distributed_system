...
int main(int argc, char *argv[]) {
  ...
  socklen_t src_addr_len = sizeof(src_addr);
	...
	while (!quit) {
		...
		n = recvfrom(sock, &RecvMsg, 33, 0, (struct sockaddr *)&src_addr, &src_addr_len); // 클라이언트로부터 메시지 송신
		if (RecvMsg.type == READ_REQ) { // 읽기 요청인 경우
			strncpy(SendMsg.key, RecvMsg.key, KEY_SIZE - 1); // key값 복사
			SendMsg.key[KEY_SIZE - 1] = '\0'; // null 문자 종료 보장
			memcpy(SendMsg.value, "AAAABBBBCCCCDDDD", 16); 
			SendMsg.type = READ_REP; // 읽기 응답으로 설정
			sendto(sock,&SendMsg,33,0,(struct sockaddr*)&src_addr, src_addr_len); // 클라이언트에 응답 전송
			printf("Type: %s Key: %s Value: %.16s\n", get_type(SendMsg), SendMsg.key,SendMsg.value); // 응답 출력
		}
	}
